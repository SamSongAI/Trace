...
- **日记 / 线程 / 文档模式** — 日记追加到当天文件，线程按主题追加到指定线程文件，文档创建独立 .md 文件，`⇧Tab` 循环切换
- **自定义分类** — 日记模式下可配置分类按钮（如"想法""待办""灵感"），对应日记中的标题
- **线程管理** — 可为不同话题预设线程及目标文件，适合连续追踪项目、问题或长期主题
...
如果你使用线程模式，也可以把线程文件配置到同一笔记库中的任意 Markdown 路径里。这样日记、线程和独立文档都能落在同一个本地工作流里。
